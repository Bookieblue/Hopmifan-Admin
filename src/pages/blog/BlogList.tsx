...
const BlogList = () => {
  ...
  const unPublishArticle = async(payload) => {
    return (await axios.post(`${baseUrl}/api/articles/unpublish`, payload))?.data
  }
  const deleteArticle = async(payload) => {
    return (await axios.post(`${baseUrl}/api/articles/delete`, payload))?.data
  }
  const publishArticleMutation = useMutation({mutationFn: publishArticle, mutationKey: ['publishArticle']})
  const unPublishArticleMutation = useMutation({mutationFn: unPublishArticle, mutationKey: ['unPublishArticle']})
  const deleteArticleMutation = useMutation({mutationFn: deleteArticle, mutationKey: ['deleteArticle']})
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    if(newStatus.toLowerCase() === 'published' || newStatus.toLowerCase() === 'publish') {
      ...
      case "publish":
        try{
          await publishArticleMutation.mutateAsync(payload, {
            onSuccess: (data) => {
              toast({
                description: data?.message || "Selected articles published successfully.."
              });
              queryClient.refetchQueries({queryKey: ['get-articles']})
            }
          });
          ...
          toast({
            description:  error?.response?.data?.message || error?.message || "Error publishing articles."
          });
        }
      ...
      case "draft":
        try{
          await unPublishArticleMutation.mutateAsync(payload, {
            onSuccess: (data) => {
              toast({
                description: data?.message || "Selected articles unpublished successfully."
              });
              queryClient.refetchQueries({queryKey: ['get-articles']})
            }
          });
          ...
          toast({
            description: error?.response?.data?.message || error?.message || "Couldn't unpublish articles."
          });
        }
        // setBlogs(blogs.map(blog =>
  ...
  const uniqueAuthors = Array.from(new Set(articlesData?.map(blog => blog.author)));
          ...
          isLoading ? <div className="flex justify-center items-center p-6">
            ...
            : isError ? (
              <div className="text-center text-red-500 p-6">
                {errorMessage ?? 'Failed to load articles. Please reload this page.'}
              </div>
            ) : <DataTable
              data={articlesData}
              columns={[
                ...
                },
                {
                  ...
                  accessor: (blog) => (
                    <div className="space-y-1 text-[14px]">
                      <div>{blog?.datePublished ? new Date(blog?.datePublished).toDateString() : 'N/A'}</div>
                      <span className={`px-2 py-1 rounded-full text-xs ${blog?.status.toLowerCase() === 'publish' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                        }`}>
                        {blog?.status}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-end gap-2 text-[14px]" onClick={(e) => e.stopPropagation()}>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-[200px] bg-white">
                          ...
                          <DropdownMenuItem
                            onClick={() => handleStatusChange(blog.id, blog?.status === 'publish' ? 'draft' : 'published')}
                          >
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => handleDeleteArticle(blogToDelete)} className="bg-red-600 hover:bg-red-700">
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
